...
pub fn read_metadata(path: &Path) -> Result<PhotoMetadata> {
    ...
    // Camera
    meta.camera_make = find_value("Make")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    meta.camera_model = find_value("Model")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    meta.lens = find_value("LensModel")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    ...
    // Dimensions
    meta.width = find_value("ImageWidth")
        .and_then(|v| v.as_i64())
        .map(|n| n as i32);
    meta.height = find_value("ImageHeight")
        .and_then(|v| v.as_i64())
        .map(|n| n as i32);
...
pub fn write_tags(path: &Path, tags: &[String]) -> Result<()> {
    ...
    let output = cmd
        .output()
        .context("Failed to run exiftool for writing tags")?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
